...
from sdk.core.bech32 import get_bech
from sdk.core.public_key import (
    PublicKey,
    address_from_public_key,
    amino_pubkey_from_public_key,
)
from sdk.core.signature_v2 import Descriptor
from sdk.core.signature_v2 import Single as SingleDescriptor
from sdk.core.tx import AuthInfo, SignerInfo, SignMode, Tx
...
class Key:
    ...
    def __init__(self, prefix: str, public_key: Optional[PublicKey] = None):
        self.prefix = prefix
    ...
    def acc_address(self) -> AccAddress:
        """Cosmos Bech32 account address. Default derivation via :data:`public_key` is provided.
    ...
    def val_address(self) -> ValAddress:
        """Cosmos Bech32 validator operator address. Default derivation via :data:`public_key` is provided.
        ...
        return ValAddress(get_bech(f"{self.prefix}valoper", self.raw_address.hex()))

    @property
    def acc_pubkey(self) -> AccPubKey:
        """Cosmos Bech32 account pubkey. Default derivation via :data:`public_key` is provided.
        ...
        return AccPubKey(get_bech(f"{self.prefix}pub", self.raw_pubkey.hex()))

    @property
    def val_pubkey(self) -> ValPubKey:
        """Cosmos Bech32 validator pubkey. Default derivation via ``public_key`` is provided.
        ...
        return ValPubKey(get_bech(f"{self.prefix}valoperpub", self.raw_pubkey.hex()))
